...
def courses(term=None):
    if term is None:
        term = 'Spring 2019'
    # order_by("-courseID") for descending order and order_by("+courseID") for ascending order.
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user.save()
        flash(f"{user.first_name}, You are registered successfully.", "success")
        return redirect(url_for('index'))
...
@app.route('/user')
def user():
    # User(user_id=1, first_name='Christian', last_name='Hur', email='christian@uta.com', password='abc123').save()
